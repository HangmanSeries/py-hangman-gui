...
class Lists(Sprite):
    ...
    def create_all_letters(self,screen,origin_x,origin_y,start_letter,end_letter,required_col):
        ...
        for i in range(start_letter,end_letter+1):
            obj = Letters(screen, chr(i), current_row, current_col, origin_x, origin_y, pygame.Color('Blue'))
    ...
    def create_word(self,screen,origin_x,origin_y,word):
        for i in range(0,len(word)):    
            obj = Letters(screen, word[i], 0, i, origin_x, origin_y, pygame.Color('Black'))
            self.append_to_list(obj)
    
    def update(self,screen,origin_x,origin_y,letter,index):
        obj = Letters(screen, letter, 0, index, origin_x, origin_y, pygame.Color('Green'))
